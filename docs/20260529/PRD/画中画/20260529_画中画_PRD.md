...
changelog: |
  - 2026-05-29：初版；Android FGS 合规驱动 + 使用时长收益；含横屏剧支持、自动连播、锁定集封面策略与 watchdog。
---
...
FlareFlow Android 端使用 `mediaPlayback` 类型的前台服务（FGS，关联 [0514 Android-FCM 高优先级透传与常驻通知](../../../20260514/PRD/Android-FCM高优先级透传唤醒与常驻通知/)）维持 FCM 透传保活、常驻通知等链路。Android 12+ 起对 FGS 的合规判定收紧——`mediaPlayback` 类 FGS 须有"用户可见 media 真在播"的证据，否则会被系统回收，进而拖垮 FCM / 保活链路。**画中画（PiP）是合规的核心证据之一**：用户切走 App 时 PiP 浮窗仍可见、视频仍在播，FGS 维持成立。

iOS 没有等价的 FGS 概念，但 PiP 同样能延长用户观看时长——用户切换 App、聊天、复制粘贴时不必关掉 FlareFlow。

因此本期把 PiP 正式做成一个独立功能，统一规则：

- **合规边界（Android 主因）**：PiP 是 FGS 续命的核心证据；无 PiP 链路则 FCM 透传/保活/常驻通知受影响。
...
| **系统最低支持** | **Android 8.0 (API 26)** / **iOS 14.0**——低于此版本设备 PiP 不可用，按现网退后台（FGS 在低版本设备降级）|
...
| 锁定集 | PiP **暂停在锁定集封面 + "Tap to unlock" overlay**；用户点 PiP 回 App 待解锁页 |
| 横屏剧支持 | **是**——进入 PiP 时根据当前集设置 aspect ratio；连播切横/竖时同步更新 |
| VIP 差异 | **无差异**（VIP 自然受益于"无锁定集"） |
| PiP 期间广告 | **全部豁免**（切集插页 / 开屏 / 激励均不触发） |
| Watchdog | PiP **实际未播放视频满 30 分钟**（停留在锁定封面）自动关闭 PiP，FGS 干净退出 |
| 本期不做 | 跨剧 PiP 连播 / PiP 内点赞收藏倍速清晰度 / 弹幕 PiP |

> **白话（触发）**：用户在沉浸播放页**按 Home 键**（Android）或**点右上角 `···` → 画中画**（手动 iOS / Android），剧集变成一个浮窗继续播放，能切上下集。

> **白话（锁定集）**：连播或点下一集时如果是付费/未解锁集，PiP 不消失也不退出，而是停在那一集的封面上，叠"Tap to unlock"提示；用户主动点 PiP 才回 App 进待解锁页。30 分钟还没动 PiP 就自动收掉，让 FGS 干净退出。
...
| LOGGED_IN（VIP / 订阅有效） | ✓ | 全集可播；**无锁定集触发 watchdog**；连播到底 |
...
flowchart TD
  ...
  A3 -->|系统权限 ON| A6[翻转切换开关偏好并持久化 ON↔OFF 不立即进 PiP]
...
flowchart TD
  ...
  B5 --> B0
  B4 -->|锁定集| B6[PiP 暂停 显示封面 + Tap to unlock overlay]
  B6 --> B7{用户行为?}
  B7 -->|点 PiP 窗体| B8[关 PiP 回 App 待解锁页]
  B7 -->|30min 内未恢复 watchdog 触发| B9[自动关 PiP FGS 退出]
  B7 -->|关闭 X / swipe 离屏| B9
```
...
| **iOS 自动触发** | App lifecycle 进入后台时，若沉浸播放器正在播放正片 + 用户切换开关偏好 = ON + 系统 PiP 权限 ON → 调用 `AVPictureInPictureController.startPictureInPicture()` |
...
**用户可控**：即使系统 PiP 权限 ON，用户也可在 App 内独立把 FlareFlow 的 PiP 关掉。点击切换开关 = **切换状态并持久化**（**不立即触发进入 PiP**）。

**状态决定矩阵**：

| 系统 PiP 权限 | 用户持久化偏好 | 切换开关显示 | 用户点击切换开关行为 |
|--------------|--------------|-----------|-----------|
| **ON** | 未设置（首次） | **ON**（默认） | 切换为 OFF 并持久化；FlareFlow PiP 关闭（按 Home 不进 PiP） |
| **ON** | **ON** | **ON** | 切换为 OFF 并持久化 |
| **ON** | **OFF**（用户曾主动关） | **OFF** | 切换为 ON 并持久化；FlareFlow PiP 恢复 |
| **OFF** | 任意 | **OFF + 置灰** | 弹标准居中权限引导弹窗（见 §5.1.3）；切换开关状态不变 |

**状态联动**：

- 系统权限 OFF → 切换开关**强制显示** OFF + 置灰（不论用户持久化偏好）
- 用户从系统设置开启权限后回到 App → 切换开关恢复显示**用户持久化偏好**（如未设置则 ON）
- 持久化存储：客户端本地（iOS NSUserDefaults / Android SharedPreferences），**不**上服务端
...
| 2 | **播放 / 暂停**（系统 `play/pause` icon） | 切换播放状态；暂停时 FGS 仍可维持，**不触发** watchdog（watchdog 仅针对锁定集封面态） |
...
**弹幕渲染策略**：PiP 期间**强制不渲染弹幕**——即使用户在全屏页打开了弹幕开关，PiP 窗内也**不显示**任何弹幕。理由：PiP 窗约 100×178 pt，弹幕字号小不易看清；强制关闭节省 CPU 与电量。用户**点 PiP 回 App 全屏**后，恢复用户原本的弹幕开关设置（PiP 期间不持久化改用户偏好）。

**音频会话策略**：PiP 期间走 OS 标准音频会话（iOS `AVAudioSession.Category.playback` / Android 媒体音频流）；与其它 App 共存规则：

- **PiP 进入瞬间**：FlareFlow 视频音轨抢占音频焦点（与全屏播放一致），其它正在播放的音乐 App（Spotify / Apple Music 等）按系统标准被暂停或音量被压低
...
```
                  ...
                  │  │                      │  │
                  │  │                      │  │
                  │  │     Tap to unlock    │  │  ← App 控制的 overlay 文字
                  │  │                      │  │
                  │  │   ⊳⊳    ⏸     ⊲⊲    │  │  ← 系统位 3 控件
  ...
  • Watchdog 从「进入此态」开始计时；30 min 内未恢复播放 → 自动关 PiP
  • 横屏剧时：窗形 16:9，封面图与 overlay 同步适配
```

| 项 | 规则 |
|----|------|
| **触发条件** | PiP 内自动连播或用户点下一集，命中**当前身份下不可免费观看**的集（含付费集未解锁、VIP 已过期遇 VIP 集等） |
| **PiP 行为** | PiP 窗**不退出**；停止视频播放；窗内显示**该锁定集的封面图**（与现网待解锁页封面一致）+ overlay 文字 **"Tap to unlock"**（中文：「点击解锁」，文案 Key `pip_locked_unlock_hint`） |
| **用户点 PiP 窗体** | 关闭 PiP → App 切到前台 → 路由到该剧该锁定集的**待解锁页**（与现网 deeplink 一致） |
| **Watchdog（FGS 风险缓解）** | PiP 状态 = "暂停在锁定集封面" 持续 **30 分钟**未恢复播放 → 客户端**自动关闭 PiP** → FGS 干净退出；用户下次回 App 沉浸播放页按现网"恢复进度"逻辑 |
| **Watchdog reset 条件** | 用户在 PiP 内点击播放 / 切到下一集 / 点 PiP 回 App，timer 立即清零；用户切到下一集仍为锁定 → 新一轮 30 分钟计时 |
...
| 触发 | 行为 |
|------|------|
| **用户点关闭 X**（系统提供） | 视频停止播放 → PiP 收起 → App 仍在后台（**不主动拉回前台**）→ FGS 退出（无 media 可播）→ 下次回 App 沉浸页按现网"恢复进度"逻辑 |
| **用户点 PiP 窗体**（非控件区） | 关闭 PiP → App 切到前台并恢复**沉浸播放页全屏**；播放进度继续；若是锁定集封面态点击则进**待解锁页** |
| **用户 swipe PiP 离屏**（Android） | 等同于点关闭 X |
| **Watchdog 30 分钟触发**（仅锁定集封面态） | 等同于点关闭 X |
...
| **锁定集封面适配** | 锁定集封面图本身有横竖两版（现网已有）→ 按集 orientation 选对应封面 + "Tap to unlock" overlay 适配 |
...
| **手动暂停** | 用户点暂停 → 视频暂停；FGS 仍可维持；**不触发 watchdog**（watchdog 仅针对锁定封面态） |
| **锁定集封面态** | 显示封面 + "Tap to unlock"；watchdog timer 开始计时 |
| **切集中** | 短暂 spinner；新集加载完毕继续播放 |
| **弱网失败** | 见 §5.3 |
| **已播完末集** | 暂停态显示"已播完"；用户点 PiP 回 App |
| **退出中** | OS 系统过渡动画；FGS 退出 |
...
Scenario: 切换开关 ON 时用户点击翻为 OFF 并持久化
...
Scenario: 切换开关 OFF 时用户点击翻为 ON 并持久化
...
Scenario: 用户双击 PiP 放大缩小不影响业务逻辑
  ...
  Then 视频应继续不中断播放
  And 连播 / 锁定集策略 / watchdog 等业务逻辑应不受影响
...
PiP 浮窗的**尺寸、位置、缩放、显隐**由 iOS / Android 系统自带的 PiP API 管控；App **不接管**任何 OS 原生手势。下表罗列**已知** OS 行为以便研发 / QA 对齐预期，**App 业务逻辑（连播、锁定集策略、watchdog 等）不受这些尺寸/位置变化影响**。

| OS 手势 / 事件 | OS 行为 | App 是否干预 |
|---------------|---------|-------------|
| **双击 PiP 浮窗** | OS 切换 PiP 尺寸（放大 ↔ 缩小，档位由系统决定） | **否**——继续正常播放 |
| **拖动 PiP 浮窗** | OS 改变 PiP 位置（吸附屏幕角落或拖到的位置） | **否** |
| **单击 PiP 浮窗 ·  控件区** | OS 显示 / 隐藏上一集 / 暂停 / 下一集三个控件 | **否** |
| **单击 PiP 浮窗 · 非控件区** | App 处理：关 PiP + 切前台 + 路由（详见 §5.6） | **是** |
...
| 沉浸播放页 | 右上角 `···` 菜单加「画中画」**切换开关项**；切换开关偏好持久化（本地存储）；进/退出 PiP 的状态管理；锁定集封面渲染；**PiP 期间弹幕渲染关闭 / 回全屏恢复** |
| 沉浸播放器（核心） | 接入 iOS `AVPictureInPictureController` + Android `PIP Mode API`；横竖屏 aspect ratio 动态切换；3 个系统位控件实现 |
| 前台服务（Android FGS） | 与 PiP 生命周期绑定；PiP 关闭时 FGS 退出；**关闭瞬间是否需要短暂续命由研发深化**（PM 边界外） |
...
| [0514 Android-FCM + 常驻通知](../../../20260514/PRD/Android-FCM高优先级透传唤醒与常驻通知/) | PiP 是本 PRD 为 FGS 提供的合规依赖；**0514 PRD 文字无需改** |
...
> - **Watchdog 30 分钟**：写死客户端常量，不做远程配置（§5.4）
> - **横屏比例**：按视频实际比例交 OS，超 OS 范围 clamp 由研发自决（§5.7）
> - **PiP 关闭瞬间 FGS 续命**：研发深化，PM 边界外（§6）
...
| 阶段 | 内容 |
|------|------|
| **本期** | iOS + Android PiP 上线；含**自动连播** + **上下集系统位** + **锁定集"Tap to unlock"** + **watchdog 30min** + **横屏剧支持** + **全广告豁免**；伴随 0513 访客 PRD §6.2 小补丁（独立 commit） |
| **Phase 2**（视数据） | watchdog 时长远程配置化；PiP 内增强控件（点赞 / 收藏，如 OS API 后续放宽） |
