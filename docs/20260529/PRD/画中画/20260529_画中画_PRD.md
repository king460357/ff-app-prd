...
changelog: |
 - 2026-05-29：初版；Android 前台服务 合规驱动 + 使用时长收益；含横屏剧支持、自动连播、锁定集封面策略与 watchdog。
---
...
FlareFlow Android 端使用 `mediaPlayback` 类型的前台服务（关联 [0514 Android-FCM 高优先级透传与常驻通知](../../../20260514/PRD/Android-FCM高优先级透传唤醒与常驻通知/)）维持 FCM 透传保活、常驻通知等链路。Android 12+ 起对前台服务的合规判定收紧——`mediaPlayback` 类前台服务须有"用户可见 media 真在播"的证据，否则会被系统回收，进而拖垮 FCM / 保活链路。**画中画（PiP）是合规的核心证据之一**：用户切走 App 时 PiP 浮窗仍可见、视频仍在播，前台服务 维持成立。

iOS 没有等价的前台服务概念，但 PiP 同样能延长用户观看时长——用户切换 App、聊天、复制粘贴时不必关掉 FlareFlow。

因此本期把 PiP 正式做成一个独立功能，统一规则：

- **合规边界（Android 主因）**：PiP 是前台服务续命的核心证据；无 PiP 链路则 FCM 透传/保活/常驻通知受影响。
...
| Watchdog | PiP **实际未播放视频满 30 分钟**（停留在锁定封面）自动关闭 PiP，前台服务 干净退出 |
...
flowchart TD
 ...
 B7 -->|30min 内未恢复 watchdog 触发| B9[自动关 PiP 前台服务退出]
 B7 -->|关闭 X / swipe 离屏| B9
```
...
| 2 | **播放 / 暂停**（系统 `play/pause` icon） | 切换播放状态；暂停时前台服务仍可维持，**不触发** watchdog（watchdog 仅针对锁定集封面态） |
...
| **Watchdog（前台服务 风险缓解）** | PiP 状态 = "暂停在锁定集封面" 持续 **30 分钟**未恢复播放 → 客户端**自动关闭 PiP** →前台服务干净退出；用户下次回 App 沉浸播放页按现网"恢复进度"逻辑 |
...
| **手动暂停** | 用户点暂停 → 视频暂停；前台服务 仍可维持；**不触发 watchdog**（watchdog 仅针对锁定封面态） |
