...
| 插页 I-01 | `ios_free_interstitial-test` | `android_free_interstitial-test` | 不插页 | 隔 X 集插页（默认 X=2） |

**子开关默认值**（未远程覆盖时）：开屏 `hot_start_enable=0`、`inter_time_sec=60`、`max_per_session=10`；插页 `start_episode=1`、`end_episode=10`、`interval_episodes=2`、`inter_time_sec=0`、`max_per_play_session=0`。详见 PRD **5.1.4 / 5.2.4**。
